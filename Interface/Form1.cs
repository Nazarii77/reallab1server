    ...
    public partial class Form1 : Form
    {
        ...
        public void Display()
        {
            ...
            if (this.renderUpdateButtons)
            {
                ...
                buttonCol.HeaderText = "Картинка";
                buttonCol.Text = "Показати";
        ...
        {
            // TODO: This line of code loads data into the 'dDDDataSet.Authors' table. You can move, or remove it, as needed.
            this.authorsTableAdapter.Fill(this.dDDDataSet.Authors);
            // TODO: This line of code loads data into the 'dDDDataSet.Books' table. You can move, or remove it, as needed.
            this.booksTableAdapter.Fill(this.dDDDataSet.Books);
            // TODO: This line of code loads data into the 'dDDDataSet.BookAuthors' table. You can move, or remove it, as needed.
            this.bookAuthorsTableAdapter.Fill(this.dDDDataSet.BookAuthors);
            this.Display();
        }
        ...
        private void createNewBookInDb(object sender, EventArgs e)
        {
            ...
            using (LibraryContext context = new LibraryContext())
            {
                ...
                book.Description = this.descriptionBox.Text;
            ...
            if (book_data_grid.Rows.Count > 0)
            {
                foreach (DataGridViewRow row in book_data_grid.SelectedRows) // foreach datagridview selected rows values  
                {
                    ...
                    else
                    {
                        ...
                        catch { id = Convert.ToInt32(row.Cells[0].Value); }
                   }
        ...
        {//Editing Book
            if (book_data_grid.Rows.Count > 0)
            {
                foreach (DataGridViewRow row in book_data_grid.SelectedRows) // foreach datagridview selected rows values  
                {
                    ...
                    {
                        try { id = Convert.ToInt32(row.Cells[1].Value); }
                        catch { id = Convert.ToInt32(row.Cells[0].Value); }
                    }
                    ...
                    using (LibraryContext context = new LibraryContext())
                    {
                        ...
                        bookToEdit.Description= this.descriptionBox.Text;
                        foreach (var item in listBox1.SelectedItems)
                        {
                            Author author = context.Authors.Where(a => a.Name == item.ToString()).Single();
